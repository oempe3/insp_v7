...
function createFieldHTML(fieldConfig, currentValue) {
    ...
    switch (fieldConfig.type) {
        ...
        case 'time':
        case 'email':
            // Para tipos de campo simples (text, number, date, time e email) o atributo type é o mesmo
            ...
            break;
        case 'textarea':
            // Usa a propriedade rows da configuração se fornecida, senão padrão 3
            const rowsAttr = fieldConfig.rows ? `rows="${fieldConfig.rows}"` : 'rows="3"';
            inputHTML = `<textarea id="${fieldId}" name="${fieldConfig.name}" ${required} placeholder="${placeholder}" ${rowsAttr} onchange="handleFieldChange('${fieldConfig.name}', this.value)">${value}</textarea>`;
            break;
        case 'select':
 ...
 */
/**
 * Inicializa campos automáticos quando um modal é aberto.  A lógica verifica
 * o array de campos definidos na estrutura do formulário e preenche valores
 * padrão conforme o atributo "auto" de cada campo.  Suporta:
 *   - start_date: preenche com a data atual no formato yyyy‑mm‑dd
 *   - start_time: preenche com a hora atual no formato hh:mm
 *   - suggest_name: recupera o último nome salvo para operador ou supervisor
 * Este método funciona tanto para formulários internos quanto externos.
 * @param {string} windowId
 */
function initializeAutomaticFields(windowId) {
    ...
    if (!config) return;
    config.fields.forEach(field => {
        const autoType = field.auto;
        if (!autoType) return;
        const fieldId = `${windowId}-${field.name}`;
        const element = document.getElementById(fieldId);
        // Se o campo já possui valor, não sobrescreve
        if (!element || element.value) return;
        if (autoType === 'start_date') {
            const today = getCurrentDate();
            element.value = today;
            handleFieldChange(field.name, today);
        } else if (autoType === 'start_time') {
            const now = getCurrentTime();
            element.value = now;
            handleFieldChange(field.name, now);
        } else if (autoType === 'suggest_name') {
            // Sugestões de nomes são aplicadas apenas para campos de operador e supervisor.
            if (field.name === 'operador' && lastNames.operador) {
                element.value = lastNames.operador;
                handleFieldChange(field.name, lastNames.operador);
            } else if (field.name === 'supervisor' && lastNames.supervisor) {
                element.value = lastNames.supervisor;
                handleFieldChange(field.name, lastNames.supervisor);
            }
        }
    });
}
